...
from OnSort import main_OnSort
...
# Chargez les images
left_images = [image.load("personages\\hero.png"), image.load("personages\\hero.png"), image.load("personages\\hero.png"), image.load("personages\\herooff.png")]
...
def draw_LeLaisser(choix1, text_box, next_button):
    global i
...
def main_LeLaisser():
    ...
    #show_buttons = False
    pygame.display.set_caption("LeLaisser")
    ...
    text_box = TextBox(0, 400, screen_width, 200)
    text_box.set_text(" \n“Il est trop loin, je ne peux pas gaspiller mes forces.”")
    ...
    draw_LeLaisser(choix1, text_box, next_button)
            ...
            if event.type == pygame.QUIT:
                ...
                if choix1.rect.collidepoint(event.pos):
                    ...
                    if next_button.action:
                        next_button.action()
                        if i == 0:
                            text_box.set_new_text("\n“Il faut que je tienne le coup et me dépêche.”")
                            draw_LeLaisser(choix1, text_box, next_button)
                        elif i == 1:
                            text_box.set_new_text("\n- Vous vous dépechez de courir pour ne pas prendre trop de retard.")
                            draw_LeLaisser(choix1, text_box, next_button)
                        elif i == 2:
