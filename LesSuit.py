...
from AttendInattentionTrident import main_AttendInattentionTrident
...
right_images = [image.load("personages\\sbireoff.png"), image.load("personages\\mechant.png"), image.load("personages\\sbire.png"), image.load("personages\\sbireoff.png")]
...
def draw_LesSuit(choix1, text_box, next_button):
    global i
...
def main_LesSuit():
    ...
    #show_buttons = False
    pygame.display.set_caption("LesSuit")
    # pygame.display.set_icon(icon)

    choix1 = Button(screen_width // 3, 220, screen_width // 3, 50, "Patienter", open_new_window_choix1)
    ...
    draw_LesSuit(choix1, text_box, next_button)
            ...
            if event.type == pygame.QUIT:
                ...
                if choix1.rect.collidepoint(event.pos):
                    ...
                    if next_button.action:
                        next_button.action()
                        if i == 0:
                            text_box.set_new_text("\n“Dépêchez vous de ranger ! On a pas toute la journée.”")
                            draw_LesSuit(choix1, text_box, next_button)
                        elif i == 1:
                            text_box.set_new_text("\n“OUI CHEF !”")
                            draw_LesSuit(choix1, text_box, next_button)
                        elif i == 2:
                            text_box.set_new_text("\n“J'ai juste à patienter pour récupérer le trident.”")
                            draw_LesSuit(choix1, text_box, next_button)
