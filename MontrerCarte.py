...
from IlSuit import main_IlSuit
...
def main_MontrerCarte():
    global i
    pygame.display.set_caption("C'est la carte, c'est la carte..")
    # pygame.display.set_icon(icon)

    # Créer une instance de TextBox et définir le texte
    text_box = TextBox(0, 400, screen_width, 200)
    son_Debalage.play()
    text_box.set_text(" \n  J’ai trouvé une carte sur la plage.\n \n  Je la suis vers le point indiqué, je suppose qu’il sera là bas.")
                ...
                if next_button.rect.collidepoint(event.pos):
                    ...
                    if next_button.action:
                        ...
                        if i == 0:
                            i += 1
                            son_Debalage.stop()
                            text_box.set_new_text("\n  Oh je vois, vous en avez de la chance !\n \n  Moi depuis que je suis là j’ai l’impression de tourner en rond..\n \n  Je peux venir avec vous ?")
                            draw_Nauffrage(text_box, next_button)
                        elif i == 1:
                            pygame.mixer.music.stop()
                            i = 0
