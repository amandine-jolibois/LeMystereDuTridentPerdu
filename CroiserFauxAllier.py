...
from MontrerCarte import main_MontrerCarte
from GarderCarte import main_GarderCarte
...
def main_CroiserFauxAllier():
    global i
    #show_buttons = False
    pygame.display.set_caption("C'est les mêmes ?!")
    # pygame.display.set_icon(icon)

    choix1 = Button(screen_width // 3, 220, screen_width // 3, 50, "Montrer la carte", open_new_window_choix1)
    choix2 = Button(screen_width // 3, 300, screen_width // 3, 50, "Garder la carte", open_new_window_choix2)

    # Créer une instance de TextBox et définir le texte
    text_box = TextBox(0, 400, screen_width, 200)
    text_box.set_text(" \n  Alors que vous avancez sur la plage vous allez tomber sur une silhouette,\n \n  un homme seul, qui vous fera signe de la main en vous voyant.")
            ...
            if event.type == pygame.QUIT:
                ...
                if choix1.rect.collidepoint(event.pos):
                    ...
                    if next_button.action:
                        next_button.action()
                        if i == 0:
                            text_box.set_new_text("\n  Hé ! Qu’est ce que vous faites ici ? Vous cherchez le trident vous aussi ?")
                            draw_Nauffrage(choix1, choix2, text_box, next_button)
                        elif i == 1:
                            text_box.set_new_text("\n  Un trident ? Ah ! Oui, c’est vrai qu’ils en parlaient, je suppose que oui du coup.")
                            draw_Nauffrage(choix1, choix2, text_box, next_button)
                        elif i == 2:
                            text_box.set_new_text("\n  “Ils” ? Vous avez du croiser ces mercenaires, il vaut mieux les éviter,\n \n  ils ont l’air dangereux.\n \n  Mais du coup, vous cherchez le trident, vous avez des indices ?")
                            draw_Nauffrage(choix1, choix2, text_box, next_button)
