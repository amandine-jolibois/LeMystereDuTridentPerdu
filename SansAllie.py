...
from PrendTrident import main_PrendTrident
...
def draw_SansAllie(choix1, text_box, next_button):
    global i
...
def main_SansAllie():
    ...
    #show_buttons = False
    pygame.display.set_caption("SansAllie")
    # pygame.display.set_icon(icon)

    choix1 = Button(screen_width // 3, 220, screen_width // 3, 50, "Prendre le Trident", open_new_window_choix1)

    # Créer une instance de TextBox et définir le texte
    text_box = TextBox(0, 400, screen_width, 200)
    text_box.set_text(" \n - Tous vos efforts n'ont pas été vains. \n \n - Vous arrivez enfin devant la salle du trésor ! \n \n - Vous entrez dans la salle du trésor, là où est posé le trident")
    ...
    draw_SansAllie(choix1, text_box, next_button)
            ...
            if event.type == pygame.QUIT:
                ...
                if choix1.rect.collidepoint(event.pos):
                    ...
                    if next_button.action:
                        ...
                        if i == 0:
                            son_BruitEcho.play()
                            text_box.set_new_text("\n“Je l’ai enfin trouvé !”\n \n“Alors comme ça il existe vraiment ce fameux trident... HAHAHAHAHA.”")
                            draw_SansAllie(choix1, text_box, next_button)
                        elif i == 1:
                            son_BruitEcho.play()
                            text_box.set_new_text("\n- Vous vous dirigez devant le trident.")
                            draw_SansAllie(choix1, text_box, next_button)
                        elif i == 2:
                            text_box.set_new_text("\n- Souhaitez-vous le prendre avec vous ?\n \n- Ce serait dommage de refuser à ce stade.")
                            draw_SansAllie(choix1, text_box, next_button)
                        i += 1
