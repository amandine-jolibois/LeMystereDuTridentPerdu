...
from RecupTrident import main_RecupTrident
...
# Chargez les images
left_images = [image.load("personages\\herooff.png"), image.load("personages\\hero.png"), image.load("personages\\hero.png"), image.load("personages\\herooff.png")]
right_images = [image.load("personages\\allié.png"), image.load("personages\\alliéoff.png"), image.load("personages\\alliéoff.png"), image.load("personages\\transparent.png")]
...
def draw_Assomer(choix1, text_box, next_button):
    global i
...
def main_Assomer():
    ...
    #show_buttons = False
    pygame.display.set_caption("Assomer")
    # pygame.display.set_icon(icon)

    choix1 = Button(screen_width // 3, 220, screen_width // 3, 50, "Récuper le trident", open_new_window_choix1)

    # Créer une instance de TextBox et définir le texte
    text_box = TextBox(0, 400, screen_width, 200)
    text_box.set_text(" \n- Vous décidez de la lancer en direction du traître,\nelle tombe à l’arrière de sa tête, le faisant tomber au sol inconscient.")
    ...
    draw_Assomer(choix1, text_box, next_button)
            ...
            if event.type == pygame.QUIT:
                ...
                if choix1.rect.collidepoint(event.pos):
                    ...
                    if next_button.action:
                        next_button.action()
                        if i == 0:
                            text_box.set_new_text("\n“C'était personnel ça par contre l'ami.”")
                            draw_Assomer(choix1, text_box, next_button)
                        elif i == 1:
                            text_box.set_new_text("\n“Mes années de baseball ont finalement servi à quelque chose.”")
                            draw_Assomer(choix1, text_box, next_button)
                        elif i == 2:
                            text_box.set_new_text("\n- Vous vous dirigez vers le trident pour le récupérer.")
                            draw_Assomer(choix1, text_box, next_button)
