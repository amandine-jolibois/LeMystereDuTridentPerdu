...
from SeCacher import main_SeCacher
from VaVoir import main_VaVoir
...
son_pasForêt = pygame.mixer.Sound("Son\\Environnement\\Bruit de pas en foret.mp3")
son_pasForêt.set_volume(0.3)
...
def main_PasserParForet():
    global i
    pygame.display.set_caption("Forêt")
    # pygame.display.set_icon(icon)

    choix1 = Button(screen_width // 3, 220, screen_width // 3, 50, "Se cacher", open_new_window_choix1)
    choix2 = Button(screen_width // 3, 300, screen_width // 3, 50, "Va voir", open_new_window_choix2)

    # Créer une instance de TextBox et définir le texte
    text_box = TextBox(0, 400, screen_width, 200)
    son_pasForêt.play()
    text_box.set_text(" \n  Vous vous enfoncez dans la forêt,\n \n  cette dernière est épaisse et humide, la température y monte rapidement.")
            ...
            if event.type == pygame.QUIT:
                ...
                if choix1.rect.collidepoint(event.pos):
                    ...
                    if next_button.action:
                        next_button.action()
                        if i == 0:
                            text_box.set_new_text("\n  Ça devient de plus en plus difficile d’avancer…\n \n  La combinaison de cette chaleur et la quantité de végétations posent problème.")
                            draw_Nauffrage(choix1, choix2, text_box, next_button)
                        elif i == 1:
                            text_box.set_new_text("\n  Alors que vous avancez dans la forêt vous entendez un bruit plus loin,\n \n  cela ressemblait à des voix humaines, elle semblaient se rapprocher.")
                            draw_Nauffrage(choix1, choix2, text_box, next_button)
                        elif i == 2:
                            son_pasForêt.stop()
