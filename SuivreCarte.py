...
from PasserParForet import main_PasserParForet
...
right_images = [image.load("personages\\chienoff.png"), image.load("personages\\chien.png"), image.load("personages\\chienoff.png"), image.load("personages\\chienoff.png")]
...
son_Mignon = pygame.mixer.Sound("Son\\Song Part2\\Chien mignion.mp3")
son_Mignon.set_volume(0.3)

son_Debalage = pygame.mixer.Sound("Son\\Song Part2\\paper.mp3")
son_Debalage.set_volume(0.4)
...
def main_SuivreCarte():
    global i
    pygame.display.set_caption("Nauffrage")
    # pygame.display.set_icon(icon)

    choix1 = Button(screen_width // 3, 220, screen_width // 3, 50, "Passer par la Fôret", open_new_window_choix1)
    choix2 = Button(screen_width // 3, 300, screen_width // 3, 50, "Passer par la Plage", open_new_window_choix2)

    # Créer une instance de TextBox et définir le texte
    text_box = TextBox(0, 400, screen_width, 200)
    son_Mignon.play()
    text_box.set_text(" \n  Le chien semble insister alors vous le suivez.\n \n  Où est-ce que tu veux m’emmener mon grand ?")
                ...
                if choix1.rect.collidepoint(event.pos):
                    ...
                    if next_button.action:
                        ...
                        if i == 0:
                            son_Mignon.stop()
                            son_pas_Chien.play()
                            text_box.set_new_text("\n  Le chien se mettra à courir devant vous avant de creuser dans le sable et en\n \n  sortira une bouteille.")
                            draw_Nauffrage(choix1, choix2, text_box, next_button)
                        elif i == 1:
                            son_pas_Chien.stop()
                            son_Debalage.play()
                            text_box.set_new_text("\n  Une bouteille ? Tu l’as senti ?\n \n  Dans la bouteille se trouve un morceau de papier, une carte, que vous déballez.\n \n  Sur la carte est représentée une île, par chance il semblerait que ce soit celle\n \n  sur laquelle vous êtes actuellement.\n \n  Un trésor ? Voilà qui rend cette île intéressante.")
                            draw_Nauffrage(choix1, choix2, text_box, next_button)
                        elif i == 2:
                            text_box.set_new_text("\n  Sur la carte est représentée un temple avec une icône de trésor, pour il allez\n \n  vous pouvez suivre la plage ou couper par la forêt.\n \n  Quel chemin empruntez vous ?")
                            son_Debalage.stop()
                            draw_Nauffrage(choix1, choix2, text_box, next_button)
