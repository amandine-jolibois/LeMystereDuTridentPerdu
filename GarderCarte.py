...
from SeSeparer import main_SeSeparer
from ResterAvec import main_ResterAvec
...
def main_GarderCarte():
    global i
    pygame.mixer.music.load("Son\\Environnement\\Embiance.mp3")
    pygame.mixer.music.play(-1)
    pygame.mixer.music.set_volume(0.1)
    #show_buttons = False
    pygame.display.set_caption("A MOI !")
    # pygame.display.set_icon(icon)

    choix1 = Button(screen_width // 3, 220, screen_width // 3, 50, "Rester avec lui", open_new_window_choix1)
    choix2 = Button(screen_width // 3, 300, screen_width // 3, 50, "Se séparer", open_new_window_choix2)

    # Créer une instance de TextBox et définir le texte
    text_box = TextBox(0, 400, screen_width, 200)
    text_box.set_text(" \n  Malheureusement je n’ai rien pour le moment, il va falloir chercher.")
            ...
            if event.type == pygame.QUIT:
                ...
                if choix1.rect.collidepoint(event.pos):
                    ...
                    if next_button.action:
                        ...
                        if i == 0:
                            i += 1
                            text_box.set_new_text("\n  Moi non plus je n’ai pas trouvé grand chose pour le moment…\n \n  Mais si vous voulez on peut s’entraider pour trouver le lieu où il est caché !")
                            draw_Nauffrage(choix1, choix2, text_box, next_button)
