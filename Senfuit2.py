...
right_images = [image.load("personages\\transparent.png"), image.load("personages\\sbireoff.png"), image.load("personages\\transparent.png"), image.load("personages\\transparent.png")]
...
def draw_Senfuit2(choix1, text_box, next_button):
    global i
...
def main_Senfuit2():
    ...
    #show_buttons = False
    pygame.display.set_caption("Senfuit2")
    # pygame.display.set_icon(icon)

    choix1 = Button(screen_width // 3, 220, screen_width // 3, 50, "Partie terminée", open_new_window_choix1)

    # Créer une instance de TextBox et définir le texte
    text_box = TextBox(0, 400, screen_width, 200)
    text_box.set_text(" \n- Vous venez de récupérer le trident.")
    ...
    draw_Senfuit2(choix1, text_box, next_button)
            ...
            if event.type == pygame.QUIT:
                ...
                if choix1.rect.collidepoint(event.pos):
                    ...
                    if next_button.action:
                        next_button.action()
                        if i == 0:
                            text_box.set_new_text("\n“hahaha merci les guignolos”")
                            draw_Senfuit2(choix1, text_box, next_button)
                        elif i == 1:
                            text_box.set_new_text("\n- Vous parvenez à quitter l’île à bord d’un bateau gonflable présent dans le sac.")
                            draw_Senfuit2(choix1, text_box, next_button)
                        elif i == 2:
                            text_box.set_new_text("\n- Par chance, votre embarcation de fortune tiendra bon jusqu’à\nrevenir sur le continent où vous pourrez étudier le trident et son histoire.")
                            draw_Senfuit2(choix1, text_box, next_button)
