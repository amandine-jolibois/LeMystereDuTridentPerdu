...
from LeLaisser import main_LeLaisser
from Assomer import main_Assomer
...
right_images = [image.load("personages\\transparent.png"), image.load("personages\\allié.png"), image.load("personages\\alliéoff.png"), image.load("personages\\alliéoff.png")]
...
def draw_Senfuit(choix1, choix2, text_box, next_button):
    global i
...
def main_Senfuit():
    ...
    #show_buttons = False
    pygame.display.set_caption("Senfuit")
    # pygame.display.set_icon(icon)

    choix1 = Button(screen_width // 3, 220, screen_width // 3, 50, "Le laisser partir", open_new_window_choix1)
    choix2 = Button(screen_width // 3, 300, screen_width // 3, 50, "L'assomer", open_new_window_choix2)

    # Créer une instance de TextBox et définir le texte
    text_box = TextBox(0, 400, screen_width, 200)
    text_box.set_text(" \n“Ouch... Aïe... Arghhhhhhhh sale fumier !”")
    ...
    draw_Senfuit(choix1, choix2, text_box, next_button)
            ...
            if event.type == pygame.QUIT:
                ...
                if choix1.rect.collidepoint(event.pos):
                    ...
                    if next_button.action:
                        next_button.action()
                        if i == 0:
                            text_box.set_new_text("\n“N'y vois rien de personnel l'ami.”")
                            draw_Senfuit(choix1, choix2, text_box, next_button)
                        elif i == 1:
                            text_box.set_new_text("\n“Il a pas loupé son coup, il faut vite que je me bouge.”")
                            draw_Senfuit(choix1, choix2, text_box, next_button)
                        elif i == 2:
                            text_box.set_new_text("\n- En vous relevant vous passez votre mains sur une pierre,\nqui a la taille parfaite pour être lancée.")
                            draw_Senfuit(choix1, choix2, text_box, next_button)
