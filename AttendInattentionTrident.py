...
from Senfuit2 import main_Senfuit2
...
# Chargez les images
left_images = [image.load("personages\\hero.png"), image.load("personages\\herooff.png"), image.load("personages\\herooff.png"), image.load("personages\\hero.png")]
right_images = [image.load("personages\\mechantoff.png"), image.load("personages\\sbireoff.png"), image.load("personages\\mechant.png"), image.load("personages\\mechantoff.png")]
...
def draw_AttendInattentionTrident(choix1, text_box, next_button):
    global i
...
def main_AttendInattentionTrident():
    global i
    pygame.mixer.music.load("Son\\Environnement\\Embiance.mp3")
    pygame.mixer.music.play(-1)
    pygame.mixer.music.set_volume(0.1)
    #show_buttons = False
    pygame.display.set_caption("AttendInattentionTrident")
    # pygame.display.set_icon(icon)

    choix1 = Button(screen_width // 3, 220, screen_width // 3, 50, "S'enfuir avec le trident", open_new_window_choix1)

    # Créer une instance de TextBox et définir le texte
    text_box = TextBox(0, 400, screen_width, 200)
    text_box.set_text(" \n“On dirait bien que je ne vais pas prendre le trident maintenant...” ")
    ...
    draw_AttendInattentionTrident(choix1, text_box, next_button)
            ...
            if event.type == pygame.QUIT:
                ...
                if choix1.rect.collidepoint(event.pos):
                    ...
                    if next_button.action:
                        next_button.action()
                        if i == 0:
                            text_box.set_new_text("\n- Alors que les sbires chargeaient le bateau leur chef admirait le trident. \n \n- Soudain un gros bruit se fit entendre, les sbires avaient fait tombé une caisse, \nleur chef posera le trident pour aller leur crier dessus.")
                            draw_AttendInattentionTrident(choix1, text_box, next_button)
                        elif i == 1:
                            text_box.set_new_text("\n“Mais qu’est ce que vous foutez bande de GUIGNOLOS !\nSi vous casser la moindre chose c’est votre salaire que je vais casser ! \nDépêchez vous et ne cassez rien !”")
                            draw_AttendInattentionTrident(choix1, text_box, next_button)
                        elif i == 2:
                            text_box.set_new_text("\n“C'est l'occasion ou jamais de prendre le trident.”")
                            draw_AttendInattentionTrident(choix1, text_box, next_button)
