...
def main_SeSeparer():
    ...
    #show_buttons = False
    pygame.display.set_caption(" Suis moi..")
    # pygame.display.set_icon(icon)

    # Créer une instance de TextBox et définir le texte
    text_box = TextBox(0, 400, screen_width, 200)
    text_box.set_text(" \n  Je préfère continuer seul, j’ai du mal à faire confiance à des inconnus.")
            ...
            if event.type == pygame.QUIT:
                ...
                if next_button.rect.collidepoint(event.pos):
                    ...
                    if next_button.action:
                        next_button.action()
                        if i == 0:
                            text_box.set_new_text("\n  Je comprend votre décision, dans ce cas je vous souhaite bon courage.")
                            draw_Nauffrage(text_box, next_button)
                        elif i == 1:
                            text_box.set_new_text("\n  Après ces mots, l’homme repartira de son côté chercher des indices.\n \n  Alors que vous continuez votre chemin pour finalement arriver\n \n  devant un temple ancien qui tombait en ruines.")
                            draw_Nauffrage(text_box, next_button)
                        elif i == 2:
                            pygame.mixer.music.stop()
                            main_FinPartie1()
