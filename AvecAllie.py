...
from Senfuit import main_Senfuit
...
background_image = pygame.image.load("fonds\\tresor.jpeg")
...
# Chargez les images
left_images = [image.load("personages\\herooff.png"), image.load("personages\\hero.png"), image.load("personages\\herooff.png"), image.load("personages\\herooff.png")]
right_images = [image.load("personages\\alliéoff.png"), image.load("personages\\allié.png"), image.load("personages\\alliéoff.png"), image.load("personages\\allié.png")]
...
son_Punch = pygame.mixer.Sound("Son\\effet sonore\\punch.mp3")
son_Punch.set_volume(0.3)
...
def draw_AvecAllie(choix1, text_box, next_button):
    global i
...
def main_AvecAllie():
    ...
    #show_buttons = False
    pygame.display.set_caption("AvecAllie")
    # pygame.display.set_icon(icon)

    choix1 = Button(screen_width // 3, 220, screen_width // 3, 50, "L'allié s'enfuit", open_new_window_choix1)

    # Créer une instance de TextBox et définir le texte
    text_box = TextBox(0, 400, screen_width, 200)
    text_box.set_text(" \n- Vous entrez dans la salle du trésor avec votre allié. \n \n- Vous vous approchez du trident.")
    ...
    draw_AvecAllie(choix1, text_box, next_button)
            ...
            if event.type == pygame.QUIT:
                ...
                if choix1.rect.collidepoint(event.pos):
                    ...
                    if next_button.action:
                        ...
                        if i == 0:
                            son_BruitEcho.play()
                            text_box.set_new_text("\n“Grâce à toi nous y voilà enfin, le trident perdu !” \n \n“On aura finit par y arriver.”")
                            draw_AvecAllie(choix1, text_box, next_button)
                        elif i == 1:
                            son_BruitEcho.play()
                            text_box.set_new_text("\n- Vous vous approchez du trident et le prenez dans vos mains. \n \n- Mais en vous retournant vers votre camarade \nvous remarquez qu’il vous jette une pierre à la tête, vous faisant tomber au sol")
                            draw_AvecAllie(choix1, text_box, next_button)
                        elif i == 2:
                            son_Punch.play()
                            son_BruitEcho.stop()
                            text_box.set_new_text("\n“Ouhhhh ça a dû faire mal ça.”\n“Ce n’est pas contre toi, c’est juste que le meilleur gagne à la fin”")
                            draw_AvecAllie(choix1, text_box, next_button)
                        i += 1
